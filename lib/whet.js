...
const getHttpMethod = (el) => {
	...
	if (specifiedTarget) return specifiedTarget
...
/**
 * @typedef {typeof enctypes[number]} Enctype
 */
const enctypes = /** @type {const} */ ([
...
/**
 * Check if the given string is a supported enctype.
 * @param {string} s 
 * @returns {s is Enctype}
 */
const isEnctype = (s) => /** @type {readonly string[]} */ (enctypes).includes(s)

/**
 * Get the content type for the request this element will send.
 * @param {Element} el the element
 * @returns {Enctype} a MIME type
 */
const getEnctype = (el) => {
	const specifiedEnctype = attr(el, 'enctype')
	if (specifiedEnctype) {
		if (isEnctype(specifiedEnctype)) {
			return specifiedEnctype
		} else {
			console.error('Element', el, 'has unsupported enctype', specifiedEnctype)
		}
	}
	return 'text/x-www-form-urlencoded'
}
...
const doElsewhereSwaps = (content) => {
	...
	for (const template of templates) {
		...
		// TODO: pass context to these events
		dispatch(target, 'whet:will-be-swapped', { isElsewhere: true })
		target?.[swapStyle](template.content)
		dispatch(target, 'whet:was-swapped-away', { isElsewhere: true })
...
const swap = (style, context, content) => {
	...
	const selector = parseSelectorFragment(context.action.hash)
	...
	dispatch(context.target, 'whet:was-swapped-away', context)
}
